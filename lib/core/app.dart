...
final _paymentDio = Dio(
  BaseOptions(
    baseUrl: 'https://pay.batela.app/api/v1',
  ),
);
...
class _AppState extends State<App> {
  ...
  Widget build(BuildContext context) {
    return RepositoryProvider(
      create: (context) =>
          PaymentRepositoryImpl(source: PaymentSource(dio: _paymentDio)),
      child: MultiBlocProvider(
        providers: [
          ...
          BlocProvider(
            create: (context) =>
                TicketBloc(savePayment: GetIt.I<SavePayment>()),
          ),
          BlocProvider(
